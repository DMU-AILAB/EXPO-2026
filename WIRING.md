...
|------|-----------|--------------|--------|------|
| 전원(종료) 버튼 | GPIO3 | 5 | 6 | 커널 기능(`dtoverlay=gpio-shutdown`), 코드 없음 |
...
| 냉각팬 (스위칭 신호) | GPIO22 | 15 | — (아래 참고) | 트랜지스터/MOSFET 통해서만 연결 |

라즈베리파이 4 GND 핀: 6, 9, 14, 20, 25, 30, 34, 39번 (아무거나 사용 가능).

---

## 1. 전원(종료) 버튼 — GPIO3 (물리 5번)
...
- 외부 저항 불필요 (커널이 풀업 처리).
- `/boot/config.txt` (Bookworm 이후는 `/boot/firmware/config.txt`)에 `dtoverlay=gpio-shutdown` 한 줄 추가 후 재부팅.
...
- 팬 정격은 12V이나 **5V로도 정상 회전 확인됨** — 별도 12V 전원 없이 Pi의 5V 핀(2/4번)으로 구동.
